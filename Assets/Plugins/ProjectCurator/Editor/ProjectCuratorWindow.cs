using Ogxd.DependencyHighlighter;
using System.IO;
using UnityEditor;
using UnityEngine;
...
public class ProjectCuratorWindow : EditorWindow, IHasCustomMenu {
    ...
    private void OnGUI() {
        ...
        GUILayout.Label(Path.GetDirectoryName(selectedPath));
        ...
        var includedStatus = ProjectCurator.GetAsset(selectedPath).IncludedStatus;
        var content = new GUIContent(selectedAssetInfo.IsIncludedInBuild ? ProjectIcons.LinkBlue : ProjectIcons.LinkBlack, includedStatus.ToString());
        ...
        if (dependenciesOpen) {
            foreach (var dependency in selectedAssetInfo.dependencies) {
                if (GUILayout.Button(Path.GetFileName(dependency), ItemStyle)) {
                    Selection.activeObject = AssetDatabase.LoadAssetAtPath<UnityEngine.Object>(dependency);
                }
                rect = GUILayoutUtility.GetLastRect();
        ...
        if (referencesOpen) {
            foreach (var referencer in selectedAssetInfo.referencers) {
                if (GUILayout.Button(Path.GetFileName(referencer), ItemStyle)) {
                    Selection.activeObject = AssetDatabase.LoadAssetAtPath<UnityEngine.Object>(referencer);
                }
                rect = GUILayoutUtility.GetLastRect();
